<?
	...
	***********************************************************************************************
	...
	$sConditions = "WHERE s.status='A' AND s.dropped!='Y' AND s.adopted='Y' AND s.qualified='Y' AND s.completed!='Y' 
	...
	$sSQL = "SELECT s.id, s.name, s.code, s.province_id, s.storey_type, s.design_type, s.work_type, 
	...
	{                       
		...
		if ($iLastProvince != $iProvince)
		{
			...
			$objPhpExcel->getActiveSheet()->setCellValue("A{$iRow}", (getDbValue("name", "tbl_provinces", "id='$iProvince'")." Active Schools List"));
			...
			$objPhpExcel->getActiveSheet()->setCellValue("H{$iRow}", "Planned Completion Date");
			$objPhpExcel->getActiveSheet()->setCellValue("I{$iRow}", "Projected Completion Date");			
			$objPhpExcel->getActiveSheet()->setCellValue("J{$iRow}", "Final Contract");
			$objPhpExcel->getActiveSheet()->setCellValue("K{$iRow}", "Blocks");
			$objPhpExcel->getActiveSheet()->setCellValue("L{$iRow}", "Class Rooms");
			$objPhpExcel->getActiveSheet()->setCellValue("M{$iRow}", "Staff Rooms");
			$objPhpExcel->getActiveSheet()->setCellValue("N{$iRow}", "Student Toilets");
			$objPhpExcel->getActiveSheet()->setCellValue("O{$iRow}", "Staff Toilets");
			$objPhpExcel->getActiveSheet()->setCellValue("P{$iRow}", "Science Labs");
			$objPhpExcel->getActiveSheet()->setCellValue("Q{$iRow}", "IT Labs");
			$objPhpExcel->getActiveSheet()->setCellValue("R{$iRow}", "Exam Halls");
			$objPhpExcel->getActiveSheet()->setCellValue("S{$iRow}", "Library");
			$objPhpExcel->getActiveSheet()->setCellValue("T{$iRow}", "Clerk Offices");
			$objPhpExcel->getActiveSheet()->setCellValue("U{$iRow}", "Princial Office");
			$objPhpExcel->getActiveSheet()->setCellValue("V{$iRow}", "Parking Stand");
			$objPhpExcel->getActiveSheet()->setCellValue("W{$iRow}", "Chowkidar Hut");
			$objPhpExcel->getActiveSheet()->setCellValue("X{$iRow}", "Soakge Pit");
			$objPhpExcel->getActiveSheet()->setCellValue("Y{$iRow}", "Water Supply");
			$objPhpExcel->getActiveSheet()->setCellValue("Z{$iRow}", "Stores");
		...
		$objPhpExcel->getActiveSheet()->setCellValue("D{$iRow}", formatDate($sStartDate, $_SESSION['DateFormat']));
		$objPhpExcel->getActiveSheet()->setCellValue("E{$iRow}", formatDate($sEndDate, $_SESSION['DateFormat']));
        $objPhpExcel->getActiveSheet()->setCellValue("F{$iRow}", formatDate($sActualStartDate, $_SESSION['DateFormat']));
		$objPhpExcel->getActiveSheet()->setCellValue("G{$iRow}", formatDate($sActualEndDate, $_SESSION['DateFormat']));
        $objPhpExcel->getActiveSheet()->setCellValue("H{$iRow}", formatDate($sPlannedCompletionDate, $_SESSION['DateFormat']));
		$objPhpExcel->getActiveSheet()->setCellValue("I{$iRow}", formatDate($sProjectedCompletionDate, $_SESSION['DateFormat']));
		$objPhpExcel->getActiveSheet()->setCellValue("J{$iRow}", 'N/A');
		$objPhpExcel->getActiveSheet()->setCellValue("K{$iRow}", $iBlocks);
		$objPhpExcel->getActiveSheet()->setCellValue("L{$iRow}", $iClassRooms);
		$objPhpExcel->getActiveSheet()->setCellValue("M{$iRow}", $iStaffRooms);
		$objPhpExcel->getActiveSheet()->setCellValue("N{$iRow}", $iStudentToilets);
		$objPhpExcel->getActiveSheet()->setCellValue("O{$iRow}", $iStaffToilets);
		$objPhpExcel->getActiveSheet()->setCellValue("P{$iRow}", $iScienceLabs);
		$objPhpExcel->getActiveSheet()->setCellValue("Q{$iRow}", $iITLabs);
		$objPhpExcel->getActiveSheet()->setCellValue("R{$iRow}", $iExamHalls);
		$objPhpExcel->getActiveSheet()->setCellValue("S{$iRow}", $iLibrary);
		$objPhpExcel->getActiveSheet()->setCellValue("T{$iRow}", $iClerkOffice);
		$objPhpExcel->getActiveSheet()->setCellValue("U{$iRow}", $iPrincipalOffice);
		$objPhpExcel->getActiveSheet()->setCellValue("V{$iRow}", $iParkingStand);
		$objPhpExcel->getActiveSheet()->setCellValue("W{$iRow}", $iChowkidarHut);
		$objPhpExcel->getActiveSheet()->setCellValue("X{$iRow}", $iSoakagePit);
		$objPhpExcel->getActiveSheet()->setCellValue("Y{$iRow}", $iWaterSupply);
		$objPhpExcel->getActiveSheet()->setCellValue("Z{$iRow}", $iStores);
		...
		{
			$objPhpExcel->getActiveSheet()->setCellValue("A{$iRow}", "Totals");
			$objPhpExcel->getActiveSheet()->setCellValue("K{$iRow}", $iTotalBlocks);
			$objPhpExcel->getActiveSheet()->setCellValue("L{$iRow}", $iTotalClassRooms);
			$objPhpExcel->getActiveSheet()->setCellValue("M{$iRow}", $iTotalStaffRooms);
			$objPhpExcel->getActiveSheet()->setCellValue("N{$iRow}", $iTotalStudentToilets);
			$objPhpExcel->getActiveSheet()->setCellValue("O{$iRow}", $iTotalStaffToilets);
			$objPhpExcel->getActiveSheet()->setCellValue("P{$iRow}", $iTotalScienceLabs);
			$objPhpExcel->getActiveSheet()->setCellValue("Q{$iRow}", $iTotalITLabs);
			$objPhpExcel->getActiveSheet()->setCellValue("R{$iRow}", $iTotalExamHalls);
			$objPhpExcel->getActiveSheet()->setCellValue("S{$iRow}", $iTotalLibrary);
			$objPhpExcel->getActiveSheet()->setCellValue("T{$iRow}", $iTotalClerkOffice);
			$objPhpExcel->getActiveSheet()->setCellValue("U{$iRow}", $iTotalPrincipalOffice);
			$objPhpExcel->getActiveSheet()->setCellValue("V{$iRow}", $iTotalParkingStand);
			$objPhpExcel->getActiveSheet()->setCellValue("W{$iRow}", $iTotalChowkidarHut);
			$objPhpExcel->getActiveSheet()->setCellValue("X{$iRow}", $iTotalSoakagePit);
			$objPhpExcel->getActiveSheet()->setCellValue("Y{$iRow}", $iTotalWaterSupply);
			$objPhpExcel->getActiveSheet()->setCellValue("Z{$iRow}", $iTotalStores);
	...
	$objPhpExcel->getActiveSheet()->getHeaderFooter()->setOddFooter("&L&B Active Schools List &R Generated on ".date("d-M-Y"));
	...
	$objPhpExcel->getActiveSheet()->setTitle("Active Schools List");
	$objPhpExcel->setActiveSheetIndex(0);
	
	
	$sExcelFile = "Active Schools.xlsx";
