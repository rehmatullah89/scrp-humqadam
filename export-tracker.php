<?
	...
	***********************************************************************************************
	...
	$sTsStagesList  = getList("tbl_stages", "id", "name", "parent_id='0' AND status='A' AND `type`='T'", "position");
	...
	// Triple Storey
	foreach ($sTsStagesList as $iParent => $sParent)
	{
		$sTsSubStages[$iParent] = "";
		...
		for ($i = 0; $i < $iCount; $i ++)
		{
			...
			if ($sChildStages == "")
			{
				$sTsSubStages[$iStage]   = $iStage;
				$sTsSubStages[$iParent] .= ((($sTsSubStages[$iParent] != "") ? "," : "").$iStage);
			}
			...
			else if ($sChildStages != "")
			{
				...
				for ($j = 0; $j < $iCount2; $j ++)
				{
					...
					if ($sChildStages == "")
					{
						$sTsSubStages[$iSubStage]  = $iSubStage;
						$sTsSubStages[$iStage]    .= ((($sTsSubStages[$iStage] != "") ? "," : "").$iSubStage);
						$sTsSubStages[$iParent]   .= ((($sTsSubStages[$iParent] != "") ? "," : "").$iSubStage);
					}

					else if ($sChildStages != "")
					{
						$sTsSubStages[$iSubStage]  = $sChildStages;
						$sTsSubStages[$iStage]    .= ((($sTsSubStages[$iStage] != "") ? "," : "").$sChildStages);
						$sTsSubStages[$iParent]   .= ((($sTsSubStages[$iParent] != "") ? "," : "").$sChildStages);
					}
				}
			}
		}
	...
	$objPhpExcel = $objReader->load("templates/tracker.xlsx");
	...
	$sSQL = "SELECT id, storey_type, design_type, province_id, name, code, class_rooms, student_toilets, covered_area, progress, ((progress / 100) * covered_area) AS _Weightage, last_stage_id, last_milestone_id
	         FROM tbl_schools
			 $sConditions
	...
	for ($i = 0; $i < $iCount; $i ++, $iRow ++)
	{
		...
		$iLastStageId = $objDb->getField($i, "last_stage_id");		
		$iLastMileStoneId = $objDb->getField($i, "last_milestone_id");		
		...
		else if ($sSchoolType == "T")
		{
			$iStatementOfRequirements = 387;
			$iCommencementLetter      = 388;
			$iSitePocessionLetter     = 389;
			$iContractDocumentation   = 391;
			$iMileStones              = 397;
			$sThirdStage              = $sTsSubStages[3];
		}		
		...
		$objPhpExcel->getActiveSheet()->setCellValueByColumnAndRow($iColumn++, $iRow, (($iLastMileStoneId > 0) ? $sAllStagesList[$iLastMileStoneId] : ""));
		$objPhpExcel->getActiveSheet()->setCellValueByColumnAndRow($iColumn++, $iRow, (($iLastStageId > 0) ? $sAllStagesList[$iLastStageId] : ""));

		$objPhpExcel->getActiveSheet()->getRowDimension($iRow)->setRowHeight(30);
		$objPhpExcel->getActiveSheet()->getStyle("A{$iRow}:R{$iRow}")->applyFromArray(array('font' => array('bold' => false, 'color' => array('rgb' => '444444'), 'size' => 13)) );
	...
	$objPhpExcel->getActiveSheet()->setCellValueByColumnAndRow(9, $iRow, (" ".formatNumber($fOverallProgress, false)."%"));

	$objPhpExcel->getActiveSheet()->getRowDimension($iRow)->setRowHeight(30);
	$objPhpExcel->getActiveSheet()->getStyle("A{$iRow}:R{$iRow}")->applyFromArray(array('fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID, 'color' => array('rgb' => '777777')),
																						'font' => array('bold' => true, 'color' => array('rgb' => 'FFFFFF'), 'size' => 14)) );
	...
	$objPhpExcel->getActiveSheet()->setTitle("Project Tracker");



	$sExcelFile = "Project Tracker.xlsx";
