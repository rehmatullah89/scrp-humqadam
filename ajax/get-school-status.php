...
<?
	...
	$iPaidAmount            = getDbValue("SUM(gross_amount)", "tbl_invoices", "school_id='$iSchool' AND status='P' AND title NOT LIKE '%Advance%'");
...
?>
		...
		<h1 class="line"><span>Financial Progress</span></h1>
                <div class="br10"></div>
                <br/>
                
                <div id="FinancialProgress">
 
<?

        $MobAdvanceAmt  = getDbValue("SUM(gross_amount)", "tbl_invoices", "school_id='$iSchool' AND title Like '%Mobilization Advance%'");
	$MobAdvanceTax  = getDbValue("SUM(tax)", "tbl_invoices", "school_id='$iSchool' AND title Like '%Mobilization Advance%'");
        $MobAdvanceDates= getDbValue("GROUP_CONCAT(date SEPARATOR ', ')", "tbl_invoices", "school_id='$iSchool' AND title Like '%Mobilization Advance%'");
        
        $TotalGrossAmt  = getDbValue("SUM(gross_amount)", "tbl_invoices", "school_id='$iSchool' AND title NOT Like '%Mobilization Advance%'");
        $TotalGrossTax  = getDbValue("SUM(tax)", "tbl_invoices", "school_id='$iSchool' AND title NOT Like '%Mobilization Advance%'");
        $TotalGrossRet  = getDbValue("SUM(retention_money)", "tbl_invoices", "school_id='$iSchool' AND title  NOT Like '%Mobilization Advance%'");
?>
  <table border="0" cellspacing="0" cellpadding="0" width="100%">
		  <tr>
                      <td><b>Mobilization Advance (PKR) :     <span><?=formatNumber($MobAdvanceAmt,false)?></span></b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<p>(Less Tax : <span  style="font-size: 11px !important;"><?=formatNumber($MobAdvanceTax, false)?></span>, &nbsp;&nbsp;&nbsp;Net Amount : <span  style="font-size: 11px !important;"><?=formatNumber(($MobAdvanceAmt-$MobAdvanceTax), false)?></span>, &nbsp;&nbsp;&nbsp;Date : <span style="color:green; font-size: 11px !important;"><?=$MobAdvanceDates?></span>)</p></td>
                  </tr>
                  <tr>
                      <td><b>Gross Amount Paid so far (PKR) : <span><?=formatNumber($TotalGrossAmt,false)?></span></b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<p>(Less Tax : <span  style="font-size: 11px !important;"><?=formatNumber(($TotalGrossTax+$TotalGrossRet), false)?></span>, &nbsp;&nbsp;&nbsp;Net Amount : <span style="font-size: 11px !important;"><?=formatNumber($TotalGrossAmt-($TotalGrossTax+$TotalGrossRet), false)?></span>)</p></td>
                  </tr>
  </table>
 <ul id="FinanceBlocks">                    
<?                    
        $sSQL = "SELECT title, gross_amount, amount FROM tbl_invoices WHERE school_id='$iSchool' AND title NOT LIKE '%Mobilization Advance%' ORDER BY id";
	$objDb->query($sSQL);
	...
	for ($i = 0; $i < $iCount; $i ++)
	{
		$sTitle     = $objDb->getField($i, "title");
                $iGrossAmt  = formatNumber($objDb->getField($i, "gross_amount"),false);
		$iNetAmount = formatNumber($objDb->getField($i, "amount"),false);
                
?>
                    <li>
                    <div>
		     <span class="progress"><?= ($i+1) ?></span>
			 <h2><?= $sTitle ?></h2>
			 <b>Gross Amt:</b> <span><?= $iGrossAmt ?> </span>
			 <br />
			 <b>Net Amt:</b> <span><?= $iNetAmount ?> </span>
		   </div>
                   </li>     
