...
<?
	...
	$sSQL = "SELECT id, type_id, name, mobile, location_time, location_address, latitude, longitude, picture
	         FROM tbl_admins
	         WHERE status='A' AND TIMESTAMPDIFF(MINUTE, location_time, NOW( )) <= '1440' $sDistrictsSQL
	         ORDER BY name";
	$objDb->query($sSQL);
	...
	for ($i = 0; $i < $iActiveInspectors; $i ++)
	{
		...
		$sInspectorInfo .= ("<td><b style='font-weight:bold;'>{$sInspector}</b><br />{$sInspectorTypes[$iType]}<br /><img src='images/icons/mobile.png' width='16' height='16' alt='' title='' align='left' style='margin-right:8px;' /> {$sMobile}<br />{$sLastInspection}<b>Last Location:</b> <small>({$sLocationDateTime})</small><br />".utf8_encode(str_replace(array("\r\n", "\n"), "<br />", htmlentities($sAddress)))."</td>");
		$sInspectorInfo .= ("</tr>");
