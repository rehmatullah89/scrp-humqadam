...
<?
	...
	$sEnumeratorTypes   = getList("tbl_admin_types", "id", "title");
	$sActiveEnumerators = "";
	...
	{
		$iEnumerator = $objDb->getField($i, "id");
		...
		$sSQL = "SELECT id, school_id, created_at FROM tbl_surveys WHERE created_by='$iEnumerator' ORDER BY id DESC LIMIT 1";
		$objDb2->query($sSQL);
		...
		if ($objDb2->getCount( ) == 1)
		{
			...
			$sLastSurvey = ("<br /><b>Last Survey:</b><br />EMIS Code: {$sSchool}<br /><a href='survey-details.php?Id={$iSurvey}' class='survey'>".formatDate($sDateTime, "d-M-Y h:i A")."</a><br /><br />");
		}


		$sEnumeratorInfo  = ("<table border='0' cellspacing='0' cellpadding='0' width='400'>");
		$sEnumeratorInfo .= ("<tr valign='top'>");
		$sEnumeratorInfo .= ("<td width='120'><div style='border:solid 1px #cccccc; padding:1px;'><img src='{$sPicture}' width='100%' alt='' title='' /></div></td>");
		$sEnumeratorInfo .= ("<td width='15'></td>");
		$sEnumeratorInfo .= ("<td><b style='font-weight:bold;'>{$sEnumerator}</b><br />{$sEnumeratorTypes[$iType]}<br /><img src='images/icons/mobile.png' width='16' height='16' alt='' title='' align='left' style='margin-right:8px;' /> {$sMobile}<br /><br />{$sLastSurvey}<b>Last Location:</b> <small>({$sLocationDateTime})</small><br />".utf8_encode(str_replace(array("\r\n", "\n"), "<br />", htmlentities($sAddress)))."</td>");
		$sEnumeratorInfo .= ("</tr>");
		$sEnumeratorInfo .= ("</table>");
?>
			var objLatLong<?= $i ?> = new google.maps.LatLng(<?= $sLatitude ?>, <?= $sLongitude ?>);
			var objMarker<?= $i ?>  = new google.maps.Marker({ position:objLatLong<?= $i ?>, map:objMap, icon:'images/map/inspector.png', title:'<?= $sEnumerator ?>', animation:google.maps.Animation.DROP });
			var objInfoWin<?= $i ?> = new google.maps.InfoWindow({ content:"<?= $sEnumeratorInfo ?>" });
