    ...
    <tr valign="top">
	  ...
	  <td width="560">
	    ...
	    <table border="0" cellspacing="0" cellpadding="5" width="100%">
	      <tr>
	        <td width="275">No of Active Inspectors in last 24 Hrs</td>
	      ...
	      <tr>
	        <td>No of Unique Schools visited in last 7 Days</td>
	        <td><a href="./" onclick="return false;"><?= formatNumber(getDbValue("COUNT(DISTINCT(school_id))", "tbl_inspections", "DATEDIFF(NOW( ), `date`) <= '7' $sInspectionsSQL"), false) ?></a></td>
	      </tr>		  
...
?>
								"<set tooltext='<?= $sInspector ?>{br}<?= $sInspectorTypes[$iType] ?>{br}{br}Total: <?= $iInspections ?>{br}Passed: <?= $iPassed ?>{br}Re-Inspections: <?= $iReInspections ?>{br}Failed: <?= $iFailed ?>{br}{br}Unique Visits: <?= $iUniqueVisits ?>' value='<?= $iPassed ?>' link='javascript:showInspections(\"<?= $iInspector ?>\", \"P\", \"\", \"\", \"\", \"\")' />" +
<?
	}
?>
								"</dataset>" +
...
<?
	for ($i = 0; $i < $iCount; $i ++)
	{
		...
		$iUniqueVisits  = $objDb->getField($i, "_UniqueVisits");
?>
								"<set tooltext='<?= $sInspector ?>{br}<?= $sInspectorTypes[$iType] ?>{br}{br}Total: <?= $iInspections ?>{br}Passed: <?= $iPassed ?>{br}Re-Inspections: <?= $iReInspections ?>{br}Failed: <?= $iFailed ?>{br}{br}Unique Visits: <?= $iUniqueVisits ?>' value='<?= $iReInspections ?>' link='javascript:showInspections(\"<?= $iInspector ?>\", \"R\", \"\", \"\", \"\", \"\")' />" +
<?
	}
?>
								"</dataset>" +
...
<?
	for ($i = 0; $i < $iCount; $i ++)
	{
		...
		$iUniqueVisits  = $objDb->getField($i, "_UniqueVisits");
?>
								"<set tooltext='<?= $sInspector ?>{br}<?= $sInspectorTypes[$iType] ?>{br}{br}Total: <?= $iInspections ?>{br}Passed: <?= $iPassed ?>{br}Re-Inspections: <?= $iReInspections ?>{br}Failed: <?= $iFailed ?>{br}{br}Unique Visits: <?= $iUniqueVisits ?>' value='<?= $iFailed ?>' link='javascript:showInspections(\"<?= $iInspector ?>\", \"F\", \"\", \"\", \"\", \"\")' />" +
<?
	}
?>
								"</dataset>" +
			...
			function showInspections(iInspector, sStatus, iProvince, iDistrict, sFromDate, sToDate)
			{
				$.colorbox({ href:("inspections.php?Inspector=" + iInspector + "&Status=" + sStatus + "&Province=" + iProvince + "&District=" + iDistrict + "&FromDate=" + sFromDate + "&ToDate=" + sToDate), width:"1100px", height:"90%", iframe:true, opacity:"0.50", overlayClose:true });
			}
...
<?
	@include("failed-inspections-reasons.php");
?>
