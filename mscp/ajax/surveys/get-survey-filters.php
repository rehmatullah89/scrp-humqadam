	...
	***********************************************************************************************
	...
	if (count($sProvincesList) > 1)
	{
		...
		foreach ($sProvincesList as $iProvince => $sProvince)
		{
			...
			$sDistrictsList = getList("tbl_districts", "id", "name", "province_id='$iProvince' AND id IN (SELECT DISTINCT(district_id) FROM tbl_surveys)");
	...
	print '<select id="SurveyStatus">';
	print '<option value="">Survey Status</option>';
    ...
    print '<select id="Qualified">';
	print '<option value="">Qualify Status</option>';
	print @utf8_encode('<option value="'.(($iSurveys > 50) ? "Y" : "Qualified").'">Qualified</option>');
	print @utf8_encode('<option value="'.(($iSurveys > 50) ? "N" : "Disqualified").'">Disqualified</option>');
	print '</select>';
