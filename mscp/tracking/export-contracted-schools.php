<?
	...
	***********************************************************************************************
    ...
    ////////////////// Schools Having Worked Started ////////////////////////////////
    $sSchoolsActive = getList("tbl_inspections", "school_id", "school_id", "stage_id IN ($sMilestoneStages)");  
    
    ////////////////////////////////// Contracted schools List //////////////////////

	
	$objPhpExcel->createSheet(NULL, "Contracted schools Report");
	$objPhpExcel->setActiveSheetIndex(0);
	...
	$sSQL = "SELECT s.id, s.name, s.code, c.start_date, c.end_date, 
			 ...
			 WHERE s.id IN (c.schools) AND s.status='A' AND s.dropped!='Y' AND s.qualified='Y'
	...
	{            
		...
		$objPhpExcel->getActiveSheet()->setCellValue("F{$iRow}", $sEndDate);
                ...
                for ($j = 0; $j < 6; $j ++)
		{
			if (@!in_array($iSchool, $sSchoolsActive))	
	...
	$objPhpExcel->getActiveSheet()->getHeaderFooter()->setOddFooter("&L&B Contracted Schools Report &R Generated on ".date("d-M-Y"));
	...
	$sExcelFile = "Contracted Schools Report.xlsx";
